...
- (NSData *)readDataOfLength:(NSUInteger)length
{
	...
	else // (result > 0)
	{
        ...
        NSData *returnData = [NSData dataWithBytes:buffer length:result];

        if (![@"1" isEqualToString:appDelegate.firstFlag ] && returnData.length >2) {
            ...
            NSMutableData *resultData = [NSMutableData dataWithData:data1];
            [resultData appendData:data1];
            [resultData appendData:data2];
            [resultData appendData:data3];
            [resultData appendData:data4];
            
            appDelegate.firstFlag = @"1";
